...
def main():
    ...
    save_path = hp_to_dirpath(
        ...
        method="TVGL", 
        dataset="synthetic", 
        include_keys=["dims", "breaks_list", "seeds_list", "alpha", "beta", "psi"],
        mkdir=True
    )
    with open(os.path.join(save_path, 'ktvgl_acc.json'), 'w') as f:
        json.dump({"Accurate": results, "Runtime": run_time}, f)
