...
def parse_args():
    ...
    # model settings
    parser.add_argument('--init_method', type=str, default='empirical', choices=['identity', 'empirical'])
    parser.add_argument('--lambdas', type=float, nargs="+", default=[0.01,0.01], help='sparsity regularization parameter')
    parser.add_argument('--max_iter', type=int, default=100, help='maximum number of iterations')
    parser.add_argument('--max_iter_gl', type=int, default=500, help='maximum number of iterations for TVGL')
...
def main():
    ...
    Thetas_hat = [[theta_m.copy() for theta_m in Thetas] for _ in range(hp['T'])]
    ...
    save_path = hp_to_dirpath(
        ...
        method="KGL", 
        dataset="synthetic", 
        include_keys=["dims", "breaks_list", "seeds_list", "lambdas", "gauge"],
        mkdir=True
    )
    with open(os.path.join(save_path, 'kgl_acc.json'), 'w') as f:
        json.dump({"Accuracy": results, "Accuracy_flatten": results_flatten , "Runtime": run_time}, f)
    with open(os.path.join(save_path, 'kgl_precision.pkl'), "wb") as f:
        pickle.dump({"Thetas_hat": Thetas_hat, "Thetas_true": Thetas_true}, f, protocol=pickle.HIGHEST_PROTOCOL)
    with open(os.path.join(save_path, 'kgl_precision_flatten.pkl'), "wb") as f:
