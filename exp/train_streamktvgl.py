...
from methods.KroneckerTVGL import StreamKroneckerTVGL, time_varying_graphical_lasso
...
def parse_args():
    ...
    # I/O setting
    parser.add_argument('--base-dir', type=str, default='../exp_results', help='base directory to save results')
    # model setting
...
def main():
    ...
    # model
    stream_ktvgl = StreamKroneckerTVGL(
        lambdas=hp['lambdas'], 
        rhos=hp['rhos'], 
    ...
    Thetas_hat_first, Thetas_hat_latest = stream_ktvgl.fit_stream(X)
    if hp["log"] == "first":
    ...
    # Evaluation
    runtimes = stream_ktvgl.runtimes
    results = ktvgl_eval(Thetas_hat, Thetas_true)


    Thetas_hat_flatten = []
    Thetas_true_flatten = []
    for t in range(len(Thetas_hat)):
    ...
    save_path = hp_to_dirpath(
        ...
        method="StreamKTVGL", 
        dataset="synthetic", 
        include_keys=["window_size", "step_size", "dims", "breaks_list", "seeds_list", "lambdas", "rhos", "init_method", "log"],
        mkdir=True
    )
    with open(os.path.join(save_path, 'StreamKTVGL_acc.json'), 'w') as f:
        json.dump({"Accuracy": results, "Accuracy_flatten": results_flatten, "Runtimes": runtimes}, f)
    with open(os.path.join(save_path, 'StreamKTVGL_precision.pkl'), "wb") as f:
        pickle.dump({"Thetas_hat": Thetas_hat, "Thetas_true": Thetas_true}, f, protocol=pickle.HIGHEST_PROTOCOL)
    with open(os.path.join(save_path, 'StreamKTVGL_precision_flatten.pkl'), "wb") as f:
